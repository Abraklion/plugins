"use strict";
(self["webpackChunkgulp_html_scss"] = self["webpackChunkgulp_html_scss"] || []).push([["select"],{
...
/* harmony import */ var _core_component__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./core/component */ "./src/js/core/component.js");
 ...
 *  Форма
 * */
class FormComponent extends _core_component__WEBPACK_IMPORTED_MODULE_3__["default"] {
  /**
   * Конструктор
  ...
  _select2() {
    ...
    const config = {
      ...
      tags: false,
      //-> динамически создавать пункты которых нет в списки. Оф дока: https://select2.org/tagging
      ...
      dropdownParent: jquery__WEBPACK_IMPORTED_MODULE_0___default()(document.body),
      //-> в каком блоке будет инициализироваться раскрывающийся список и относительно этого блока позицианироваться
    ...
    const configMultiple = {
      ...
      //-> включает режим множественного выбора
      allowClear: true,
      //-> появляется кнопка [x] которая сбрасывает все выбраные пункты
      ...
      closeOnSelect: false,
      //-> предотвращающая закрытие раскрывающегося списка при выборе

      maximumSelectionLength: 0,
      //-> макс. количество элементов, которые могут быть выбраны. по умолчанию: 0
     ...
     * */
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-division').select2({
      placeholder: 'выберите дивизион',
      // -> текст по умолчанию. по умолчанию: null

      data: data,
      // -> конфигурации из локального массива
     ...
     * */
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-department').select2({
     ...
     * */
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-post').select2({
      placeholder: 'выберите должность',
      // -> текст по умолчанию. Не нужен пустой option в html. по умолчанию: null
     ...
     * */
    let selectCertificate = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-certificate');
    ...
    selectCertificate.on('select2:opening select2:closing', function (event) {
      let $searchfield = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).parent().find('.select2-search__field');
     ...
     * */
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-select-class').select2({
      ajax: {
        // все остальные настройки как у jQuery.ajax
        dataType: 'json',
        //-> в каком формате хотим получить данные

        url: 'data/select-multiple.json',
        //-> url куда отправляем
     ...
     * Стаж список | Ajax
     * */
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.js-select-age').select2({
      ajax: {
        // все остальные настройки как у jQuery.ajax
        dataType: 'json',
        //-> в каком формате хотим получить данные
     ...
     * */
    let $selectAdditions = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-additions');
     ...
     * */
    let $selectValue = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-value');
    $selectValue.select2({
      placeholder: 'выберите значения',
      ...config,
      disabled: true
    });
    let mapApi = ['select-addon-blood.json', 'select-family-status.json', 'select-residence.json'];
  ...
  _validate() {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.sForm__select').on('change.select2', function () {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).prev('.sForm__label').trigger('click');
    });
  ...
  if (this.form.form()) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.quantity').text('');

        // сбросить кастомные селекты
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.sForm__select').val(null).trigger('change');

        // заблокировать кастомные селект
        jquery__WEBPACK_IMPORTED_MODULE_0___default()('.select-value').prop('disabled', true);
      }, 1000);
    }
  }
}
...
/******/ __webpack_require__.O(0, ["select2","jquery-validation","jquery"], function() { return __webpack_exec__("./src/js/select2.js"); });
